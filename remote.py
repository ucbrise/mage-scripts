...
def exec_sync(ip_address, command, check_exitcode = False):
    result = subprocess.run(("ssh", "-i", "mage", "mage@{0}".format(ip_address), command))
...
def exec_async(ip_address, command, get_output = True):
    ...
    result = subprocess.Popen(("ssh", "-o", "StrictHostKeyChecking=no", "-i", "mage", "mage@{0}".format(ip_address), command), stdout = stdout_arg, stderr = stderr_arg)
    return result
